/*
 *@# AttachmentHandler.java 6:22:42 PM Jul 22, 2009
...
import com.gm.core.io.IOUtils;
import com.gm.mail.reciver.util.Global;
...
public class AttachmentHandler {
	...
	protected static List<Attachment> extractAttachments(Part part) throws Exception {
		...
		try {
			fileName = StringUtils.isBlank(part.getFileName()) ? null : MimeUtility.decodeText(part.getFileName())
					.trim();
		...
		if (part.isMimeType("multipart/*")) {//多媒体格式
			...
			if (fileName != null || disposition != null
					&& (disposition.equals(Part.ATTACHMENT) || disposition.equals(Part.INLINE)) && fileName != null) {
				//将附件写进磁盘
				//存储文件名
				...
				IOUtils.saveFile(fullFilePath, part.getInputStream());
