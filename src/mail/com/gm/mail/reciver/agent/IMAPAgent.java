/*
 *@# POP3Agent.java 2:04:32 PM Jul 21, 2009

 */
package com.gm.mail.reciver.agent;
...
import com.gm.mail.reciver.MailMessage;
 ...
 */
public class IMAPAgent implements MailAgent {
	...
	public boolean login(String folderName) throws Exception {
		...
		store.connect(this.setting.getHostAddress(), this.setting.getPort(), this.setting.getUserName(),
				this.setting.getPwd());
		//设置Folder成员
	 ...
	 * @param setting
	 */
	public void setReciverSetting(Setting setting) {
	...
	public List<MailMessage> fetchUnSeenMailMessage() throws Exception {
		...
		//查找未读邮件列表
		Message[] msgs = folder
				.search(new AndTerm(new SearchTerm[] { new FlagTerm(new Flags(Flags.Flag.SEEN), false) }));
		FetchProfile fp = new FetchProfile();
		fp.add(UIDFolder.FetchProfileItem.UID);
		folder.fetch(msgs, fp);
